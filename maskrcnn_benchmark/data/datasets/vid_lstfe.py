...
class VIDLSTFEDataset(VIDDataset):
    def __init__(self, image_set, data_dir, img_dir, anno_path, img_index, transforms, is_train=True):
        super(VIDLSTFEDataset, self).__init__(image_set, data_dir, img_dir, anno_path, img_index, transforms, is_train=is_train)
        if not self.is_train:
            self.start_index = []
            self.start_id = []
            if cfg.MODEL.VID.LSTFE.GLOBAL.ENABLE:
                self.shuffled_index = {}
            for id, image_index in enumerate(self.image_set_index):
                ...
                if frame_id == 0:
                    ...
                    if cfg.MODEL.VID.LSTFE.GLOBAL.ENABLE:
                        ...
                        if cfg.MODEL.VID.LSTFE.GLOBAL.SHUFFLE:
    ...
    def _get_train(self, idx):
        ...
        if hasattr(self, "pattern"):
            offsets = np.random.choice(cfg.MODEL.VID.LSTFE.MAX_OFFSET - cfg.MODEL.VID.LSTFE.MIN_OFFSET + 1,
                                       cfg.MODEL.VID.LSTFE.REF_NUM_LOCAL, replace=False) + cfg.MODEL.VID.LSTFE.MIN_OFFSET
            ...
            # global frames
            if cfg.MODEL.VID.LSTFE.GLOBAL.ENABLE:
                ref_ids = np.random.choice(self.frame_seg_len[idx], cfg.MODEL.VID.LSTFE.GLOBAL_FRAME_NUM, replace=False)
                for ref_id in ref_ids:
                    ref_filename = self.pattern[idx] % ref_id
                    img_ref = Image.open(self._img_dir % ref_filename).convert("RGB")
                    img_refs_g.append(img_ref)
        else:
            for i in range(cfg.MODEL.VID.LSTFE.REF_NUM_LOCAL):
                img_refs_l.append(img.copy())
            if cfg.MODEL.VID.LSTFE.GLOBAL.ENABLE:
                for i in range(cfg.MODEL.VID.LSTFE.REF_NUM_GLOBAL):
    ...
    def _get_test(self, idx):
        ...
        ref_id = min(self.frame_seg_len[idx] - 1, frame_id + cfg.MODEL.VID.LSTFE.MAX_OFFSET)
        ref_filename = self.pattern[idx] % ref_id
        img_ref = Image.open(self._img_dir % ref_filename).convert("RGB")
        img_refs_l.append(img_ref)
        ...
        img_refs_g = []
        if cfg.MODEL.VID.LSTFE.GLOBAL.ENABLE:
            size = cfg.MODEL.VID.LSTFE.GLOBAL.SIZE if frame_id == 0 else 1
            shuffled_index = self.shuffled_index[str(self.start_id[idx])]
            for id in range(size):
                filename = self.pattern[idx] % shuffled_index[
                    (idx - self.start_id[idx] + cfg.MODEL.VID.LSTFE.GLOBAL.SIZE - id - 1) % self.frame_seg_len[idx]]
                img = Image.open(self._img_dir % filename).convert("RGB")
