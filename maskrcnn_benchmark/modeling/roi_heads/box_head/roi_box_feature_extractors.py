...
from maskrcnn_benchmark.modeling.make_layers import Conv2d
...
@registry.ROI_BOX_FEATURE_EXTRACTORS.register("LSTFEFeatureExtractor")
class LSTFEFeatureExtractor(AttentionExtractor):
    def __init__(self, cfg, in_channels):
        super(LSTFEFeatureExtractor, self).__init__(cfg, in_channels)
        stage = resnet.StageSpec(index=4, block_count=3, return_features=False)
        head = resnet.ResNetHead(
        ...
        if self.global_enable:
            self.global_size = cfg.MODEL.VID.LSTFE.GLOBAL.SIZE
            self.global_res_stage = cfg.MODEL.VID.LSTFE.GLOBAL.RES_STAGE
    ...
    def _forward_test_single(self, i, cache):
        rois_cur = cache.pop("rois_cur")
        rois_ref = cache.pop("rois_ref")
        feats_cur = cache.pop("feats_cur")
        feats_ref = cache.pop("feats_ref")
    ...
    def _forward_train(self, x, proposals):
        proposals, proposals_l, proposals_g = proposals
        x_l, x_g = x
        ...
        # update current feats
        for i in range(self.stage):
            feats = self._forward_train_single(i, self.local_cache[i], ver="local")
    ...
    def _forward_test(self, x, proposals):
        ...
        for i in range(self.stage):
            ...
            feat_cur = self._forward_test_single(i, self.local_cache[i])
